...
function parseArgs() {
  ...
  const out = { list: false, sourceTitle: '', sourceId: null, outPath: '' }
...
function main() {
  ...
  if (opts.list) {
    ...
    console.log(JSON.stringify({ totalTitles: list.length, titles: list }, null, 2))
    return
  }
  ...
  if (!sourceTitle && sourceId === null) {
    console.error('缺少参数：--source-id 70（推荐，避免中文编码问题）或 --source-title "xxx"（或先用 --list）')
    process.exit(2)
  }
  ...
  const defaultOut = path.join('C:\\Temp\\textbook-mappings', `backlog_${sanitizeFilename(pickedTitle)}.csv`)
  const outPath = opts.outPath ? opts.outPath : defaultOut
  fs.mkdirSync(path.dirname(outPath), { recursive: true })
  fs.writeFileSync(outPath, toCsv(rows), 'utf8')

  console.log('[export_citation_backlog_csv] done', {
    sourceTitle: pickedTitle,
    sourceId: sourceId !== null ? sourceId : undefined,
    rows: rows.length,
    out: outPath,
  })
}
